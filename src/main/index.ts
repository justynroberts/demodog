...
app.whenReady().then(() => {
  ...
  app.on('activate', () => {
    // Clicking the Dock icon has to bring the window back, and this only ever
    // made a *new* one when none existed — so a minimised window stayed
    // minimised and a hidden one stayed hidden, with the app apparently
    // refusing to come back at all. Nothing was broken; nothing was listening.
    if (!studioWindow || studioWindow.isDestroyed()) {
      createStudioWindow()
      return
    }
    // Except mid-take. The studio is hidden on purpose while recording, and
    // showing it here would put the app's own window into the recording.
    if (recorder) return
...
/**
 * Puts the control bar up in its "ready" state.
 *
 * The same bar that carries Stop during a take, rather than a second floating
 * thing beside it: one panel that changes what it offers is less to look at
 * and lands in the place the user already associates with the recording.
 */
ipcMain.handle('ready:show', async (_e, detail: { title: string; hint: string }) => {
